...
const SignUpPage = () => {
  ...
  const validateForm = () => {
    if (!formData.fullName.trim()) return toast.error("Le nom entier est demandé")
      if (!formData.email.trim()) return toast.error("L'adresse email est requise")
      ...
      if (!formData.password) return toast.error("Le mot de passe est requis")
        if (formData.password.length < 6) return toast.error("Un minimum de six caractères est requis pour le mot de passe");
      return true;
  };
